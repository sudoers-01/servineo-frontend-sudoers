...
export default function AccountLoginSettings({ token }: Props) {
  ...
  const handleLink = async (provider: string) => {
    try {
      ...
      console.error(err);
      alert(`Error al vincular el método ${provider}: ${err instanceof Error ? err.message : "Desconocido"}`);
    }
  };

  const handleUnlink = async (provider: string) => {
    if (linkedMethods.length <= 1) {
      alert("⚠️ Debes tener al menos un método activo.");
      return;
    }
    if (window.confirm(`¿Desvincular ${provider}?`)) {
      try {
        ...
        console.error(err);
        alert(`Error al desvincular método ${provider}: ${err instanceof Error ? err.message : "Desconocido"}`);
      }
    }
  };
