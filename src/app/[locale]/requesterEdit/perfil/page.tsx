...
export default function MiPerfilPage() {
  ...
  const updateEditableFields = (
    data: IUser & { firstName?: string; lastName?: string }
  ) => {
...
if (!isValidEmail(editEmail.trim())) {
  ...
  return;
      ...
      const photoToSend = editPhotoFile ? base64Photo : (userData?.photo ?? userData?.picture ?? userData?.url_photo ?? null);
      ...
      if (photoToSend && photoToSend.trim() !== '' && photoToSend !== 'null' && photoToSend !== 'undefined') {
      ...
      if (!res.ok) {
        ...
        throw new Error(errorData.message || errorData.error || `Error ${res.status}: No se pudo actualizar el perfil`);
      }
