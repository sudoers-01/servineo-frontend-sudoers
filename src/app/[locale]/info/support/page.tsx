  ...
  return (
    <main className="bg-[var(--background)] text-[var(--foreground)] min-h-screen">
      ...
      <section className="max-w-6xl mx-auto py-8 px-6 md:px-10 space-y-6">
        {faqs.map((category, i) => (
          <div key={i} className="bg-[var(--background)] rounded-xl shadow-lg border border-[var(--light-gray)] overflow-hidden">
      ...
      <section className="bg-[var(--light-gray)] py-10 px-6 md:px-10">
        <div className="max-w-6xl mx-auto grid md:grid-cols-3 gap-5">
          {contactOptions.map((option, i) => (
            <div key={i} className="bg-[var(--background)] p-6 rounded-xl shadow-lg hover:shadow-xl transition-all duration-300">
              <div className={`w-12 h-12 bg-[var(--primary)] rounded-lg flex items-center justify-center mb-4`}>
                <svg className="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">{option.icon}</svg>
              </div>
